...
LOGREG_DIR = Path("save/logistic_regression_multi_tissue_regul")
...
hyperparameter_defaults = dict(
    ...
    dab_weight=0.0,
    lr=3e-4,  # Higher initial learning rate for faster convergence
    batch_size=64,  # Increased batch size for faster training and more stable gradients
    layer_size=128,  # Increased model capacity
    nlayers=4,  # Reduced layers for faster training (4 is often sufficient)
    nhead=5,  # More attention heads
    dropout=0.4,  # Reduced dropout for better learning
    weight_decay=2e-4,  # Reduced weight decay for less regularization
    ...
    early_stopping_patience=5,  # Increased patience to allow more training
    label_smoothing=0.1,  # Reduced label smoothing for better learning
    use_reduce_lr_on_plateau=True,  # Use ReduceLROnPlateau for better LR scheduling
    reduce_lr_factor=0.7,  # Less aggressive LR reduction (0.7 instead of 0.5)
    reduce_lr_patience=3,  # More patience before reducing LR (3 instead of 1)
    min_lr=1e-5,  # Higher minimum learning rate to keep learning
)
# %%
...
# Speed optimizations: disable deterministic for faster training
...
max_seq_len = 900  # Can reduce if sequences are shorter (e.g., 512 or 768) for speed
...
for tissue in TISSUES:
    ...
    base_save_dir = Path(f"save/scgpt_multi_tissue_logreg_filtered")
    ...
    # Load and filter aging-related DEGs using the same logic as compare_logistic_regression_multi_tissue.py
    ...
    if not ageanno_genes_path.exists():
        ...
        results_summary.append({"tissue": tissue, "status": "error", "error": f"File not found: {ageanno_genes_path}"})
        continue
    
    genes = pd.read_csv(LOGREG_DIR / tissue / "coefficients.csv")
    ...
    logger.info(f"Filtering to {len(genes_to_keep)} aging-related DEGs that are present in both train and test data")
    
    if len(genes_to_keep) == 0:
        logger.error(f"No aging-related DEGs found in the data for tissue {tissue}. Skipping...")
    ...
    def prepare_dataloader(
        ...
        # Enable num_workers for faster data loading
        num_workers = config.get("num_workers", 8)
        if num_workers == 0:
            num_workers = min(len(os.sched_getaffinity(0)) if hasattr(os, 'sched_getaffinity') else 8, batch_size // 2, 8)
    ...
    # Speed optimization: Enable memory efficient attention if available
