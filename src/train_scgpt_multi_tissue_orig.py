...
from sklearn.metrics import accuracy_score, roc_auc_score, roc_curve, precision_score, recall_score, f1_score, classification_report, confusion_matrix as sk_confusion_matrix
from tqdm import tqdm
...
TISSUES = [
    "skin", "bladder", "blood", "bone-marrow", "brain", 
    "liver", "lung", "skeletal-muscle", "stomach"
]
...
hyperparameter_defaults = dict(
    ...
    batch_size=100,  # Increased batch size for more stable gradients
    ...
    early_stopping_patience=3,  # Increased patience to allow recovery
...
# Enable cudnn benchmark for faster training (slightly less reproducible but much faster)
...
for tissue in TISSUES:
    ...
    logger.info(f"Starting training for tissue: {tissue}")
    ...
    if not train_path.exists():
        ...
        results_summary.append({"tissue": tissue, "status": "error", "error": f"File not found: {train_path}"})
        continue
    if not test_path.exists():
        logger.error(f"Test file not found: {test_path}")
        results_summary.append({"tissue": tissue, "status": "error", "error": f"File not found: {test_path}"})
        continue
    ...
    logger.info(f"Filtering to {len(available_genes)} genes from top 1000 that are present in both train and test data")
    
    if len(available_genes) == 0:
        logger.error(f"No genes from top 1000 found in the data for tissue {tissue}. Skipping...")
        results_summary.append({"tissue": tissue, "status": "error", "error": "No matching genes found"})
        continue
    
    # Filter X_train and X_test to only include genes that exist in the data
    adata_train = adata_train[:, available_genes]
    adata_test = adata_test[:, available_genes]
    
    # Calculate number of genes and adjust hyperparameters accordingly
    num_genes = len(available_genes)
    logger.info(f"Using {num_genes} genes for training tissue: {tissue}")
    
    # Reference: previously used ~870 genes as baseline
    reference_genes = 870
    gene_ratio = num_genes / reference_genes if reference_genes > 0 else 1.0
    logger.info(f"Gene ratio (current/reference): {gene_ratio:.3f} ({num_genes}/{reference_genes})")
    
    # Adjust hyperparameters based on gene count for better learning
    # More genes = more features to learn, so we need:
    # 1. Slightly higher learning rate (logarithmic scaling to avoid instability)
    # 2. Potentially adjusted batch size for memory
    # 3. Adjusted weight decay for regularization
    # 4. Updated max_seq_len to accommodate genes
    tissue_config = config.copy()  # Create tissue-specific config
    
    if gene_ratio > 1.0:
        # Scale learning rate logarithmically (less aggressive than linear)
        lr_scale = 1.0 + 0.15 * np.log(gene_ratio)  # e.g., 1.5x genes -> ~1.06x LR
        tissue_config["lr"] = tissue_config["lr"] * lr_scale
        logger.info(f"Adjusted learning rate for {tissue}: {tissue_config['lr']:.6f} (scaled by {lr_scale:.3f})")
        
        # Adjust batch size if needed (more genes = more memory)
        if gene_ratio > 1.3:
            tissue_config["batch_size"] = max(32, int(tissue_config["batch_size"] * 0.95))
            logger.info(f"Adjusted batch size for {tissue}: {tissue_config['batch_size']} (reduced for memory)")
        
        # Very small weight decay - minimal regularization
        # tissue_config["weight_decay"] remains at very small value
        logger.info(f"Using very small weight decay for {tissue}: {tissue_config['weight_decay']:.6f}")
    elif gene_ratio < 0.8:
        # Fewer genes - can use slightly higher LR and larger batch
        lr_scale = 1.0 + 0.1 * (1.0 - gene_ratio)
        tissue_config["lr"] = tissue_config["lr"] * lr_scale
        logger.info(f"Adjusted learning rate for {tissue}: {tissue_config['lr']:.6f} (scaled by {lr_scale:.3f} for fewer genes)")
    
    # Update max_seq_len to accommodate genes (with some buffer)
    max_seq_len = min(2048, int(num_genes * 1.15) + 1)  # +1 for CLS token, 15% buffer
    logger.info(f"Adjusted max_seq_len for {tissue}: {max_seq_len}")
    ...
    def compute_roc_auc(model, adata_test, save_plot=True):
        ...
        all_probs: Probabilities for positive class
        all_labels: True labels
        cell_ids: Cell IDs in the same order as predictions
        """
        model.eval()
    
        # Get cell IDs before processing (they will be in the same order as data)
        cell_ids = adata_test.obs_names.tolist()
    
        # Prepare test data
        test_data, test_age_labels, test_batch_labels = return_data_age_batch(adata_test)
    
        # Tokenize test data
        tokenized_test = tokenize_and_pad_batch(test_data,
            gene_ids,
            max_len=max_seq_len,
            vocab=vocab,
            pad_token=pad_token,
        ...
        masked_values_test = random_mask_value(tokenized_test["values"],
        ...
        test_loader = prepare_dataloader(test_data_pt,
        ...
        return roc_auc, all_probs, all_labels, cell_ids
    ...
    logger.info(f"Hyperparameters saved to {save_dir / 'hyperparameters.json'}")
    ...
    # Use best_model for evaluation if available, otherwise use current model
    model_to_eval = best_model if best_model is not None else model
    logger.info(f"Evaluating model for ROC-AUC and other metrics...")
    roc_auc_score_test, test_probs, test_labels, test_cell_ids = compute_roc_auc(model_to_eval, adata_test, save_plot=False)
    ...
    logger.info(f"Test predictions saved to {save_dir / 'test_predictions.json'}")

    # Calculate predictions from probabilities (using 0.5 as threshold for binary classification)
    ...
    cm = sk_confusion_matrix(test_labels, test_preds)
    
    # Get classification report
    class_report = classification_report(test_labels, test_preds, target_names=['Mid', 'Old'], output_dict=True)
    ...
    results_dict = {
        "tissue": tissue,
        "model": "scGPT",
        "roc_auc": float(roc_auc_score_test),
        "accuracy": float(accuracy),
        "precision": float(precision),
        "recall": float(recall),
        "f1": float(f1),
        "n_features": int(adata_test.shape[1]),
        "n_train_samples": int(adata_train.shape[0]),
        "n_test_samples": int(adata_test.shape[0]),
        "train_class_distribution": {
            "mid (0)": int(np.sum(train_age_labels == 0)),
            "old (1)": int(np.sum(train_age_labels == 1))
        },
        "test_class_distribution": {
            "mid (0)": int(np.sum(test_labels == 0)),
            "old (1)": int(np.sum(test_labels == 1))
        },
    ...
    # Plot and save ROC curve (ensure it's saved)
    plot_roc_curve(test_labels, test_probs, roc_auc_score_test,
                title=f'ROC Curve for Age Classification - {tissue.upper()}',
                save_path=save_dir / "roc_curve.png",
                show_plot=False)
    logger.info(f"ROC curve saved to {save_dir / 'roc_curve.png'}")
