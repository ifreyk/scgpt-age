...
# Output directory
save_dir = Path("save/logistic_regression_multi_tissue_regul_orig")
...
def prepare_data(adata_train, adata_test):
    ...
    adata_train.obs["age_id"] = [
        1 if x == "old" else 0 for x in adata_train.obs["age_category"]
    ]
...
def train_logistic_regression(
    X_train, y_train, X_test, y_test, tissue: str, gene_names: List[str], C: float = 1.0
):
    """
    ...
    print(
        f"Training labels - Class 0 (mid): {np.sum(y_train == 0)}, Class 1 (old): {np.sum(y_train == 1)}"
    )
    print(
        f"Test labels - Class 0 (mid): {np.sum(y_test == 0)}, Class 1 (old): {np.sum(y_test == 1)}"
    )
    ...
    return {
        ...
        "classification_report": classification_report(
            y_test, y_pred, target_names=["Mid", "Old"], output_dict=True
        ),
...
# %%
def plot_roc_curve_multi_tissue(
    tissue_results: Dict[str, Dict], save_path=None, show_plot=True, figsize=(12, 10)
):
    """
    ...
    for idx, (tissue, results) in enumerate(tissue_results.items()):
        ...
        plt.plot(
            ...
            color=colors[idx],
            lw=2.5,
            label=f"{tissue.upper()} (AUC = {roc_auc:.4f})",
            alpha=0.8,
        )
    ...
    plt.ylabel("True Positive Rate (Sensitivity)", fontsize=14, fontweight="bold")
    plt.title(
        "ROC Curves: Logistic Regression (L1 Regularized) - Multiple Tissues",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
# %%
def save_tissue_results(
    tissue: str, results: Dict, X_train, X_test, y_train, y_test, tissue_save_dir: Path
):
    """
    ...
    results_dict = {
        ...
        "regularization": {
            ...
            "sparsity_percentage": float(
                results["n_zero_coef"] / X_train.shape[1] * 100
            ),
        },
...
if __name__ == "__main__":
    ...
    ageanno_genes = ageanno_genes[
        (ageanno_genes["isTissuespecific"] == False)
        & (ageanno_genes["isCellTypespecific"] == False)
    ]
    ...
    for tissue in TISSUES:
        try:
            ...
            gene_indices = [
                i for i, gene in enumerate(gene_names) if gene in unique_genes
            ]
            ...
            results = train_logistic_regression(
                X_train, y_train, X_test, y_test, tissue, filtered_gene_names, C=1.0
            )
            ...
            save_tissue_results(
                tissue, results, X_train, X_test, y_train, y_test, tissue_save_dir
            )
