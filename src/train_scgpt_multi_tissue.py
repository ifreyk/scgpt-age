...
hyperparameter_defaults = dict(
    ...
    nlayers=5,  # More layers for better representation
...
for tissue in TISSUES:
    ...
    logger.info(f"Filtered to {len(unique_genes)} aging-related DEGs (old vs mid)")
    ...
    if len(missing_genes) > 0:
        logger.warning(f"{len(missing_genes)} genes from aging-related DEGs are not present in the data. First 10 missing: {missing_genes[:10]}")
    
    logger.info(f"Filtering to {len(available_genes)} aging-related DEGs that are present in both train and test data")
